...
$tender_id = (int)$_GET['id'];
$tender = $db->QuerySingleRowArray("SELECT `end_date` FROM `tenders` WHERE `id` = " . $tender_id, MYSQL_ASSOC);

// Counting down to New Year's on 2020
$countdown_to = $tender['end_date']; // 24-Hour Format: YYYY-MM-DD HH:MM:SS"
...
function convert_to_time(secs)
{
	...
	if (dd == 0) { return (hh+":"+mm+":"+ss); }
		...
		if (dd > 1) { return (dd+" дней "+hh+" часов "+mm+" минут "+ss+" секунд"); }
		else { return (dd+" день "+hh+" часов "+mm+" минут "+ss+" секунд"); }
	}
}
...
function do_cd()
{
	if (countdown < 0)
	{
		<?php
			...
			{
		?>
		document.location.href = "/tenders/show/<?php echo $tender_id; ?>";
		<?php } ?>
	}
	else
	{
		document.getElementById('countdown-finish').innerHTML = convert_to_time(countdown);
		setTimeout('do_cd()', 1000);
	}
	countdown = countdown - 1;
}

document.write("<div id='countdown-finish'></div>\n");
