...
class Tank_auth
{
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login))) {	// login ok
				...
				if ($password == $user->password) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						$this->error = array('banned' => 1);

					} else {
						$user_profile = $this->ci->users->get_user_profile($user->id);
						$user_group = $this->ci->users->get_user_group($user_profile->group_id);
						...
						if ($user->activated == 0) {							// fail - not activated
							$this->error = array('not_activated' => 1);
	...
	{
		return $this->ci->session->userdata('group_title');
	}
/*
	function new_pass($id, $password)
	{
		...
		$this->ci->db->query("UPDATE `users` SET `password` = '" . $hashed_password . "' WHERE `id` = " . (int)$id);
	}
*/
	...
	function create_user_from_admin($username, $email, $password, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			$this->error = array('username' => 'auth_username_in_use');
			...
			if (!is_null($res = $this->ci->users->create_user_from_admin($data, !$email_activation))) {
				$data['user_id'] = $res['user_id'];
				$data['password'] = $password;
				unset($data['last_ip']);
				return $data;
	...
	/**
	 * Create new user profile on the site and return bool value.
	 *
	 * @param	array
	 * @return	bool
	 */
	...
	{
		if (!is_null($this->ci->users->create_profile($data)))
			return TRUE;
		else
			return FALSE;
	}
	...
	/**
	 * Update data user profile on the site and return bool value.
	 *
	 * @param	int
	 * @param	array
	 * @return	bool
	...
	{
		if (!is_null($this->ci->users->update_profile($user_id, $data)))
			return TRUE;
		else
			return FALSE;
	}
	...
	/**
	 * Update data user and return bool value.
	 *
	 * @param	int
	 * @param	array
	 * @return	bool
	...
	function change_password($old_pass, $new_pass)
	{
		...
		if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
			...
			if ($old_pass == $user->password) {			// success

				// Replace old password with new one
				$this->ci->users->change_password($user_id, $new_pass);
				return TRUE;
	...
	function users_list($confirm = FALSE, $banned = FALSE)
	{
		$list = $this->ci->users->get_users($confirm, $banned);
	...
	{
		$data = $this->ci->users->get_user($user_id);
